...
class CellDetector:
        ...
        for x in range(slide.width):
            for y in range(slide.height):
                if working_slide.get_pixel(x, y) > 0:
                    cell_pixels, working_slide = self.explore_cell(
                        working_slide, x, y)
                    # ignore cells that are too small
                    if len(cell_pixels) > effective_threshold:
    ...
    def explore_cell(self, slide, base_x, base_y):
        ...
        :returns cell_pixels: a list of all the (X, Y) pixel coordinates that
    ...
    def print_cell_results(cells):
        ...
        :param cells: a list of detected cells, where each cell is list of
         (X, Y) pixel coordinates that contain the cell
        :returns None
        '''
